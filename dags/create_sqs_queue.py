...
# Creating SQS queue using boto3 client and subscribing to SNS topic
def create_sqs_queue_and_subscribe(queue_name):
    sqs_client = boto3.client('sqs', region_name='us-east-2')
    sns_client = boto3.client('sns', region_name='us-east-2')
    
    # SNS topic arn  - provide the corresponding variable set in Airflow for SNS topic you wish to subscribe 
    topic_arn = Variable.get("<SNS_VARIABLE>")
    ...
    # Define the policy to subscribe to the SNS topic 
    policy = {
        "Version": "2012-10-17",
...
# Define DAG to create SQS queue and subscribe to SNS
with DAG(
    ...
    create_queue_and_subscribe = PythonOperator(
        ...
        op_kwargs={'queue_name': '<SQS_QUEUE_NAME>'} # provide your preferred sqs queue name
    )
